...
async function getLogsByUser(userId, userSectionId, userClassId) {
  try {
    const url2 = new URL(`${LOG_ENDPOINT}/${userId}`);
    if (userSectionId) {
      url2.searchParams.append("user_section_id", userSectionId);
    }
    if (userClassId) {
      url2.searchParams.append("user_class_id", userClassId);
    }
...
async function getUserStatus(userId, classId) {
  try {
    ...
    const url2 = new URL(`${USER_ENDPOINT}/${userId}/class-status`);
    if (classId) {
      url2.searchParams.append("class_id", classId);
    }
    const response = await fetch(url2.toString(), {
      method: "GET",
      headers: { "Content-Type": "application/json" }
...
async function getUserSection(userId, classId) {
  try {
    const url2 = new URL(`${USER_ENDPOINT}/${userId}/sections`);
    if (classId) {
      url2.searchParams.append("class_id", classId);
    }
    const response = await fetch(url2.toString(), {
...
var http_default = isHttpAdapterSupported && function httpAdapter(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    req = transport.request(options, function handleResponse(res) {
      ...
      const lastRequest2 = res.req || req;
      ...
      if (responseType === "stream") {
        ...
        responseStream.on("error", function handleStreamError(err) {
          if (req.destroyed) return;
          reject(AxiosError_default.from(err, null, config, lastRequest2));
        });
...
async function notifyUser(message, url2, isModal = false) {
  ...
  vscode2.window.showInformationMessage(message, { modal: isModal }, "Review", "Ignore").then((selection) => {
    if (selection === "Review") {
      vscode2.env.openExternal(
        vscode2.Uri.parse(url2 || "https://clover.nickrucinski.com/")
      );
    }
  });
}
...
async function signInWithGithub() {
  try {
    const session = await vscode3.authentication.getSession("github", ["read:user"], { createIfNone: true });
    if (!session || !session.accessToken) {
      await errorNotification("GitHub Sign In failed: No session or access token found.");
      await authNotification();
      return;
    }
    const resp = await axios_default.get("https://api.github.com/user", {
      headers: {
    ...
    vscode3.window.showInformationMessage(`Signed in as ${username}`);
    try {
      await axios_default.post("https://backend-639487598928.us-east5.run.app/api/auth/github", {
...
var isSuspended;
var getPromptText = (document2, position) => {
  const language = document2.languageId;
  const fullText = document2.getText();
  const cursorOffset = document2.offsetAt(position);
...
var setLastRequest = (document2, position, context, token) => {
  lastRequest = { document: document2, position, context, token };
};
...
async function provideInlineCompletionItems(document2, position, context, token) {
  ...
  setLastRequest(document2, position, context, token);
...
var CollabAgentPanelProvider = class {
  ...
  sessionManager = null;
  _usernameKey = "collabAgent.username";
  _username;
          ...
          case "requestGithubSignInThenStart":
            ...
            (async () => {
              ...
              if (signed) {
                ...
                vscode13.window.showWarningMessage("GitHub sign-in cancelled or failed.");
  ...
  async sendStoredUsernameToWebview() {
    const stored = this._context.globalState.get(this._usernameKey);
  ...
  async clearUsername() {
    ...
    await this._context.globalState.update(this._usernameKey, void 0);
  ...
  async startLiveShareSession() {
    const signed = await this.ensureGithubAuthenticated();
    if (!signed) {
      vscode13.window.showWarningMessage("GitHub sign-in is required to start a session with your GitHub name.");
      return;
    }
  ...
  async joinLiveShareSession() {
    const signed = await this.ensureGithubAuthenticated();
    if (!signed) {
      vscode13.window.showWarningMessage("GitHub sign-in is required to join a session with your GitHub name.");
      return;
    }
    if (!this._liveShareApi) {
      vscode13.window.showErrorMessage("Live Share API not available. Please install Live Share extension.");
   ...
   */
  async ensureGithubAuthenticated() {
    try {
      const session = await vscode13.authentication.getSession("github", ["read:user"], { createIfNone: true });
      if (!session || !session.accessToken) {
        return false;
      }
      try {
        ...
        vscode13.window.showErrorMessage('Failed to fetch GitHub profile. Please ensure GitHub sign-in is working or install the "GitHub Pull Requests and Issues" extension.');
        return false;
      }
    } catch (err) {
      ...
      if (choice === "Open Extensions") {
        await vscode13.commands.executeCommand("workbench.extensions.search", "GitHub Pull Requests and Issues");
      }
      return false;
    }
  }
