...
var LOCAL_ENDPOINT_URL, ENDPOINT_URL, TESTING, BASE_URL, AUTH_ENDPOINT, AI_SUGGESTION_ENDPOINT, REFINE_PROMPT_ENDPOINT, EXPLANATION_ENDPOINT, LOG_SUGGESTION_ENDPOINT, LOG_LINE_SUGGESTION_ENDPOINT, HINT_ENDPOINT, LOG_ENDPOINT, USER_ENDPOINT, ANSWER_ENDPOINT;
var init_endpoints = __esm({
  "src/api/types/endpoints.ts"() {
    "use strict";
    LOCAL_ENDPOINT_URL = "http://127.0.0.1:8001";
    ENDPOINT_URL = "https://backend-639487598928.us-east5.run.app";
    TESTING = true;
    BASE_URL = TESTING ? LOCAL_ENDPOINT_URL : ENDPOINT_URL;
    AUTH_ENDPOINT = `${BASE_URL}/auth`;
    AI_SUGGESTION_ENDPOINT = `${BASE_URL}/suggestion`;
    REFINE_PROMPT_ENDPOINT = `${BASE_URL}/suggestion/refine`;
    EXPLANATION_ENDPOINT = `${BASE_URL}/suggestion/explanation`;
    LOG_SUGGESTION_ENDPOINT = `${BASE_URL}/logs/suggestion`;
    LOG_LINE_SUGGESTION_ENDPOINT = `${BASE_URL}/logs/line-suggestion`;
    HINT_ENDPOINT = `${BASE_URL}/suggestion/hint`;
    LOG_ENDPOINT = `${BASE_URL}/logs`;
    USER_ENDPOINT = `${BASE_URL}/users`;
    ANSWER_ENDPOINT = `${BASE_URL}/suggestion/answer`;
  }
});
...
function getSupabase() {
  ...
  const url = (vscode2.workspace.getConfiguration().get("collabAgent.supabase.url") || process.env.SUPABASE_URL || "").trim();
  const anonKey = (vscode2.workspace.getConfiguration().get("collabAgent.supabase.anonKey") || process.env.SUPABASE_ANON_KEY || process.env.SUPABASE_KEY || "").trim();
  if (!url || !anonKey) {
  ...
  client = (0, import_supabase_js.createClient)(url, anonKey, { auth: { persistSession: true, autoRefreshToken: true } });
  return client;
}
...
var import_supabase_js, vscode2, client;
var init_supabaseClient = __esm({
  "src/auth/supabaseClient.ts"() {
    "use strict";
    import_supabase_js = __toESM(require_main5());
...
__export(auth_service_exports, {
  ...
  handleSignUpProvided: () => handleSignUpProvided,
  setAuthContext: () => setAuthContext,
  signInMenu: () => signInMenu,
  signInOrUpMenu: () => signInOrUpMenu,
...
async function handleSignIn() {
  ...
  vscode3.commands.executeCommand("collabAgent.authStateChanged");
}
...
async function signInWithGithub() {
  try {
    const { getSupabase: getSupabase2 } = (init_supabaseClient(), __toCommonJS(supabaseClient_exports));
    ...
    if (data?.url) {
      await vscode3.env.openExternal(vscode3.Uri.parse(data.url));
    } else {
...
function createAuthStatusBarItem(context) {
  const authStatusBarItem = vscode4.window.createStatusBarItem(
    vscode4.StatusBarAlignment.Right,
    100
  );
  authStatusBarItem.name = "Collab Agent Authentication";
  authStatusBarItem.backgroundColor = new vscode4.ThemeColor(
  ...
  context.subscriptions.push(
    authStatusBarItem,
    vscode4.commands.registerCommand("collabAgent.showAuthOptions", async () => {
      ...
      if (choice === "Sign In with GitHub") {
        vscode4.commands.executeCommand("collabAgent.githubLogin");
      } else if (choice === "Sign In with Email") {
        vscode4.commands.executeCommand("collabAgent.emailLogin");
      } else if (choice === "Sign Up") {
        vscode4.commands.executeCommand("collabAgent.signUp");
      }
    }),
    vscode4.commands.registerCommand("collabAgent.authStateChanged", updateAuthStatus)
  );
  return authStatusBarItem;
}
var vscode4, signInCommand, signOutCommand, handleAuthUri, uriHandlerCommand;
var init_auth_commands = __esm({
  "src/commands/auth-commands.ts"() {
    ...
    init_auth_service();
    init_notifications();
    signInCommand = vscode4.commands.registerCommand(
      "collabAgent.signIn",
      async () => signInOrUpMenu()
    );
    signOutCommand = vscode4.commands.registerCommand(
      "collabAgent.signOut",
      async () => signOutMenu()
    );
    handleAuthUri = async (uri) => {
      if (uri.path === "/auth-complete") {
        ...
        try {
          ...
          await showAuthNotification(`Sign In successfully! \u{1F389}`);
          vscode4.commands.executeCommand("collabAgent.authStateChanged");
        } catch (err) {
      ...
      if (uri.path === "/auth/callback") {
        try {
          ...
          const { setAuthContext: setAuthContext2 } = (init_auth_service(), __toCommonJS(auth_service_exports));
          const { error: ctxErr } = await setAuthContext2(minimalUser);
          if (ctxErr) {
    ...
    uriHandlerCommand = vscode4.window.registerUriHandler({
