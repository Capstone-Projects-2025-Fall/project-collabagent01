...
describe("Auth Service", () => {
  ...
  describe("signInWithGithub", () => {
    it("should open GitHub authentication URL", async () => {
    ...
    it("should show error if GitHub sign in fails", async () => {
      mockOpenExternal.mockRejectedValue(new Error("GitHub error"));
      await signInWithGithub();
      expect(errorNotification).toHaveBeenCalledWith(
        expect.stringContaining("GitHub Sign In failed")
      );
    });
  });
  ...
  describe("handleSignOut", () => {
    it("should sign out successfully", async () => {
      ...
      await handleSignOut();
      expect(mockUpdate).toHaveBeenCalledWith(AUTH_CONTEXT, undefined);
      expect(showAuthNotification).toHaveBeenCalledWith(
        "Sign Out Successfully! 👋"
      );
      expect(mockExecuteCommand).toHaveBeenCalledWith(
        "clover.authStateChanged"
      );
    });

    it("should show error if sign out fails", async () => {
      mockGet.mockImplementation(() => {
